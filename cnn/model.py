...
#                    nn.BatchNorm1d(opt.channels),
#                    nn.ReLU(True),
#                    nn.Conv1d(opt.channels, 1, 5, padding=2),
                    nn.ReLU(True))
#                    nn.Conv2d(opt.channels, opt.channels, (3, 64), padding=(1, 0)),
#                    nn.ReLU(True))
...
#        evidence_embedding = self.get_evidence_embedding(batch, aligned_feature)
        ...
        question_embed_view = question_embedding.view(batch_size * e_max_len, q_emb_size)

        evidence_embedding = evidence_embedding.contiguous()
        # (batch, e_len, e_size) -> # (batch * e_len, e_size)
        evidence_embed_view = evidence_embedding.view(batch_size * e_max_len, e_emb_size)

        # (batch * e_len, q_size) (batch * e_len, e_size) -> batch * e_len
        start_score_bilinear = self.start_matcher.forward(evidence_embed_view, question_embed_view)
        end_score_bilinear = self.end_matcher.forward(evidence_embed_view, question_embed_view)
        # (batch, e_len)
...
#        conv1_start = self.pooling(conv1_start).squeeze(-1)
#        conv1_end = self.pooling(conv1_end).squeeze(-1)
#        conv2_start = self.conv_start_2(conv1_start)
#        conv2_end = self.conv_end_2(conv1_end)
        # (batch, e_len, channel)
